...
testnet testnetOptions H.Conf {..} = do
  ...
  H.rewriteYamlFile (tempAbsPath </> "configuration.yaml") . J.rewriteObject
    ...
    . HM.insert "ByronGenesisFile" (J.toJSON @String "genesis/byron/genesis.json")
    . HM.insert "ShelleyGenesisFile" (J.toJSON @String "genesis/shelley/genesis.json")
    . HM.insert "AlonzoGenesisFile" (J.toJSON @String "genesis/shelley/genesis.alonzo.json")
    ...
    . HM.insert "TestEnableDevelopmentHardForkEras" (J.toJSON True)
  ...
  H.lbsWriteFile (tempAbsPath </> "node-spo1/topology.json") $ J.encode $
    J.object
    [ "Producers" .= J.toJSON
      [ J.object
        [ "addr"    .= J.toJSON @String "127.0.0.1"
        , "port"    .= J.toJSON @Int 3002
        , "valency" .= J.toJSON @Int 1
        ]
      , J.object
        [ "addr"    .= J.toJSON @String "127.0.0.1"
        , "port"    .= J.toJSON @Int 3003
        , "valency" .= J.toJSON @Int 1
        ]
      ]
    ]
  
  H.lbsWriteFile (tempAbsPath </> "node-spo2/topology.json") $ J.encode $
    J.object
    [ "Producers" .= J.toJSON
      [ J.object
        [ "addr"    .= J.toJSON @String "127.0.0.1"
        , "port"    .= J.toJSON @Int 3001
        , "valency" .= J.toJSON @Int 1
        ]
      , J.object
        [ "addr"    .= J.toJSON @String "127.0.0.1"
  ...
  H.lbsWriteFile (tempAbsPath </> "node-spo3/topology.json") $ J.encode $
    J.object
    [ "Producers" .= J.toJSON
      [ J.object
        [ "addr"    .= J.toJSON @String "127.0.0.1"
